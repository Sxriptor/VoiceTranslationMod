...
const hearYourselfButton = document.getElementById('hear-yourself-button');
...
const bidirectionalInputSelect = document.getElementById('bidirectional-input-select');
...
let bidirectionalInputDeviceId = null;
...
function setupRealTimeAudioPlayback() {
    ...
    window.electronAPI.setupRealTimeAudioPlayback(() => { });
}
...
async function playTestAudio(audioData, outputToHeadphones) {
    try {
        ...
        if (!outputToHeadphones && outputToVirtualDevice && virtualOutputDeviceId && 'setSinkId' in audio) {
            try {
                await audio.setSinkId(virtualOutputDeviceId);
...
function initializeEventListeners() {
    ...
    if (bidirectionalInputSelect)
        bidirectionalInputSelect.addEventListener('change', onBidirectionalInputChange);
...
function showApiKeyModal(apiKeys) {
    ...
    modalContent.querySelector('#save-btn')?.addEventListener('click', async () => {
        const openaiKey = modalContent.querySelector('#openai-key').value;
        const elevenlabsKey = modalContent.querySelector('#elevenlabs-key').value;
        await updateApiKeys({
            openai: openaiKey,
...
async function restoreSidebarPreference() {
    try {
        const response = await window.electronAPI.invoke('config:get', {
            id: Date.now().toString(),
            timestamp: Date.now(),
...
async function loadBidirectionalInputDevices() {
    if (!bidirectionalInputSelect)
        return;
    try {
...
async function onBidirectionalInputChange() {
    if (!bidirectionalInputSelect)
        return;
...
async function refreshDevicesAndVoices() {
    try {
...
async function updateApiKeys(apiKeys) {
    try {
        const response = await window.electronAPI.invoke('config:set', {
            id: Date.now().toString(),
            timestamp: Date.now(),
            payload: { apiKeys }
        });
...
function setupTranslationStatusUpdates() {
    ...
    const statusInterval = setInterval(async () => {
        ...
        try {
            const response = await window.electronAPI.invoke('pipeline:get-status', {
                id: Date.now().toString(),
                timestamp: Date.now(),
                payload: null
            });
...
async function testHearYourself() {
    try {
...
async function showAddVoiceModal() {
    ...
    modal.appendChild(modalContent);
    document.body.appendChild(modal);
    const voiceIdInput = modalContent.querySelector('#voice-id-input');
    const voiceNameInput = modalContent.querySelector('#voice-name-input');
    const testVoiceCheckbox = modalContent.querySelector('#test-voice-checkbox');
    const testVoiceBtn = modalContent.querySelector('#test-voice-btn');
    const addVoiceBtn = modalContent.querySelector('#add-voice-btn');
    const cancelBtn = modalContent.querySelector('#cancel-voice-btn');
...
async function addCustomVoiceToList(voiceId, displayName) {
...
async function saveCustomVoice(voiceId, displayName) {
    try {
        // Get current config
        const response = await window.electronAPI.invoke('config:get', {
            id: Date.now().toString(),
...
async function playAudioInRenderer(audioBufferArray) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (outputToVirtualDevice && virtualOutputDeviceId && 'setSinkId' in audio) {
                audio.setSinkId(virtualOutputDeviceId).then(() => {
...
async function translateText(text) {
    try {
        const response = await window.electronAPI.invoke('translation:translate', {
            id: Date.now().toString(),
            timestamp: Date.now(),
            payload: {
                text,
...
async function synthesizeAndPlay(text) {
    try {
        const response = await window.electronAPI.invoke('tts:synthesize', {
            id: Date.now().toString(),
            timestamp: Date.now(),
            payload: {
                text,
...
async function detectVirtualOutputDevice() {
    try {
        const devices = await navigator.mediaDevices.enumerateDevices();
        const outputs = devices.filter(d => d.kind === 'audiooutput');
...
async function restoreOutputPreference() {
    try {
        const response = await window.electronAPI.invoke('config:get', {
            id: Date.now().toString(),
            timestamp: Date.now(),
...
function updateOutputToggleButton() {
...
async function toggleOutputTarget() {
